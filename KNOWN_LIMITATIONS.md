...
This document describes limitations for JotLuck `v0.12.3-preview`. These are not
marketing claims; they are the remaining release constraints and expected
behavior boundaries.

## Preview Status

- `v0.12.3-preview` is the current public, unsigned preview of both the
...
- The current Windows preview is unsigned. Its only official public download is
  the `v0.12.3-preview` GitHub Release; verify the SHA-256 published on that page
  before installation.
