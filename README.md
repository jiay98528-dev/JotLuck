...
JotLuck is a lightweight, local-first Markdown notebook. Each note is a plain
...
Current release channel: `v0.1.0-preview`.
...
pnpm.cmd --filter @jotluck/app preview
```
...
```powershell
pnpm.cmd --filter @jotluck/app tauri:build
```
...
External `.md/.markdown/.mdx/.txt` files opened from Windows start in an
independent, single-file read-only window. “Enable editing” keeps the session
limited to that file; “Add to notes” explicitly promotes its parent folder to a
notebook. The installer registers JotLuck as an optional Open With handler and
does not force it as the system default application.
...
pnpm.cmd --filter @jotluck/app build
pnpm.cmd --filter @jotluck/app exec playwright test --project=chromium --workers=1
pnpm.cmd --filter @jotluck/app exec playwright test --project=firefox --workers=1
pnpm.cmd --filter @jotluck/app exec playwright test --project=webkit --workers=1
