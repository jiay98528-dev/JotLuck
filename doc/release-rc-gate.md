...
从 `v0.11.0` 起，Web 自动化全绿只能表述为“自动化候选通过”，不能表述为“签名申请通过”或“最终发布通过”。签名申请候选必须额外满足真实安装包 L4；完成签名后还必须验证 Authenticode 与签名后 SHA-256。历史报告中的旧版本号保持历史事实，不作迁移。
...
- 安装版 L4: 必须基于 `JotLuck_0.11.0_x64-setup.exe` 安装后的真实应用。

## 执行顺序

1. 运行既有自动化: L1/L2/coverage/build/Rust check/test、三引擎 E2E、`tauri:build`。
2. 生成并定位 `JotLuck_0.11.0_x64-setup.exe`。默认路径为 `packages/app/src-tauri/target/release/bundle/nsis/JotLuck_0.11.0_x64-setup.exe`；文件必须小于 10 MiB。
...
5. 将候选 commit 冻结并推送到 `main`，以 `workflow_dispatch` 运行完整 CI，并提供唯一安全的 `release_id`。capture job 只有在 L1、Vitest、Rust、三引擎 E2E、Windows 视觉、Tauri build/smoke 全部成功后，才从同一 run 的候选 artifact 安装并执行固定 adapter；任一 case 失败、skip、缺执行日志或观察产物时不上传 evidence artifact，只保留失败诊断。
...
- 分别双击 `.docx/.pdf/.xlsx/.xls` -> 观察渐进语义预览 -> 验证只有“启动编辑”一个主入口，并可选择检测到的专业软件编辑原件。
- 对导入文档选择“编辑 Markdown 副本” -> 确认原件不会修改 -> 原生 Save As 强制 `.md` -> 成功后进入 JotLuck 编辑会话；DOCX 图片资产写入不冲突的伴随目录，源文件 SHA-256 保持不变。
- 在 Save As 对话框返回后、首个目标变更前修改源文件 -> 保存必须拒绝旧转换并要求重新转换；专业软件修改原件后也必须标记过期。
- 搜索 -> 点击结果 -> 编辑命中笔记。
...
- 卸载 -> 验证安装目录、`.md/.markdown/.mdx/.txt/.docx/.pdf/.xlsx/.xls` 可选文件关联、OpenWith 残留和图标残留；安装前后的系统默认应用必须一致。
...
- 只有安装版 L4 全部 PASS、闸门脚本 PASS、精确候选提交与安装器哈希已绑定，才能称为“签名申请候选通过”。只有签名服务返回的同一产物通过 Authenticode 与签名后哈希复核，才能称为“签名版本发行通过”。

## 变更记录

- 2026-08-05：升级为 `v0.11.0` 签名候选闸门；扩展为八种 Windows 打开方式，新增 DOCX/PDF/XLSX/XLS 渐进预览、专业软件编辑、Save As Markdown、源文件竞态与资产目录验证；补充签名前后身份闭环和 10 MiB 上限。
