...
- 环境层是低色度暖灰纸面，不叠加多层全屏彩雾；蓝紫与珊瑚只能出现在 glass chrome 的局部折射伪层。
- 材质高度固定为三档：环境无阴影；主 chrome 使用轻浮起、透光、内侧高光与 12px blur，侧栏收敛为 8px blur；工具分组只使用浅内凹；正文画布使用最强但克制的接触与扩散阴影。
...
- 2026-07-25：Halo Canvas 环境收敛为低色度暖灰纸面，主 chrome/侧栏模糊统一为实现事实源的 12px/8px。
